...
```bash
rust-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rust-app$ sam local start-api
rust-app$ curl http://localhost:3000/
```
...
```bash
rust-app$ sam logs -n HelloWorldFunction --stack-name rust-app --tail
```
